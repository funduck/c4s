        ...
        processMessage: function (event, data) {
            switch (event) {
                ...
                case "ch:ls:actions" : // data == {callback, filter = {<name>:<value>}}
                    this.sendMessage(data.callback, {data: aoj_extract(aoj_select(this.character.avacts, data.filter), "id")});
                    break;
                case "ch:get:action" : // data == {callback, filter = {<name>:<value>}}
                    this.sendMessage(data.callback, {data: aoj_select(this.character.avacts, data.filter)[0]});
                    break;
                default :
